...
export const likePost = async (postId) => {
  ...
  if (response.status === 200) {
    ...
    console.log("[ERROR] Unauthorized - Please log in to like a post");
  } else {
    console.log("[ERROR] Wrong post ID");
  }
  window.location.reload(); // like 시 새로고침으로 반영
};
...
export const deleteComment = async (id) => {
  // Django REST Framework 기본 삭제 응답은 204
  const response = await instanceWithToken.delete(`/comment/${id}/`);
  if (response.status !== 204) {
    throw new Error("DELETE_COMMENT_FAILED");
  }
  window.location.reload(); // 코멘트 삭제 시 새로고침으로 반영
  return response;
};
